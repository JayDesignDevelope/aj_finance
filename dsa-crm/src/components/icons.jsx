...
// ── Label icons (replace emoji) ───────────────────────────────────
export const IcFlame = (p) => (<svg {...I(p)}><path d="M8.5 14.5A2.5 2.5 0 0 0 11 12c0-1.38-.5-2-1-3-1.072-2.143-.224-4.054 2-6 .5 2.5 2 4.9 4 6.5 2 1.6 3 3.5 3 5.5a7 7 0 1 1-14 0c0-1.153.433-2.294 1-3a2.5 2.5 0 0 0 2.5 2.5z"/></svg>);
export const IcSun = (p) => (<svg {...I(p)}><circle cx="12" cy="12" r="4"/><path d="M12 2v2M12 20v2M4.93 4.93l1.41 1.41M17.66 17.66l1.41 1.41M2 12h2M20 12h2M6.34 17.66l-1.41 1.41M19.07 4.93l-1.41 1.41"/></svg>);
export const IcSnow = (p) => (<svg {...I(p)}><line x1="12" y1="2" x2="12" y2="22"/><line x1="2" y1="12" x2="22" y2="12"/><path d="m20 16-4-4 4-4M4 8l4 4-4 4M16 4l-4 4-4-4M8 20l4-4 4 4"/></svg>);
export const IcStar = (p) => (<svg {...I(p)}><polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"/></svg>);
export const IcRefresh = (p) => (<svg {...I(p)}><polyline points="23 4 23 10 17 10"/><polyline points="1 20 1 14 7 14"/><path d="M3.51 9a9 9 0 0 1 14.85-3.36L23 10M1 14l4.64 4.36A9 9 0 0 0 20.49 15"/></svg>);
export const IcTag = (p) => (<svg {...I(p)}><path d="M20.59 13.41l-7.17 7.17a2 2 0 0 1-2.83 0L2 12V2h10l8.59 8.59a2 2 0 0 1 0 2.82z"/><line x1="7" y1="7" x2="7.01" y2="7"/></svg>);
export const IcNote = (p) => (<svg {...I(p)}><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg>);
export const IcSparkle = (p) => (<svg {...I(p)}><path d="M12 3l1.9 5.8L19.5 11l-5.6 2.2L12 19l-1.9-5.8L4.5 11l5.6-2.2L12 3z"/></svg>);
export const IcFile = (p) => (<svg {...I(p)}><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/></svg>);
export const IcMenu = (p) => (<svg {...I(p)}><line x1="3" y1="12" x2="21" y2="12"/><line x1="3" y1="6" x2="21" y2="6"/><line x1="3" y1="18" x2="21" y2="18"/></svg>);

// ── Mappers: label key / activity type → icon component ───────────
export const LABEL_ICON = { hot: IcFlame, warm: IcSun, cold: IcSnow, priority: IcStar, reengage: IcRefresh };
export function LabelIcon({ label, size = 14, ...p }) {
  const C = LABEL_ICON[label];
  return C ? <C width={size} height={size} {...p} /> : null;
}
const ACTIVITY_ICON = {
  created: IcSparkle, assigned: IcAgents, stage: IcChart, outcome: IcPhone, label: IcTag,
  note: IcNote, whatsapp: IcWhats, email: IcMail, sms: IcSms, callback: IcClock,
};
export function ActivityIcon({ type, size = 15, ...p }) {
  const C = ACTIVITY_ICON[type] || IcSparkle;
  return <C width={size} height={size} {...p} />;
}
